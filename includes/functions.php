...
/*
		if(file_exists("../pictures/".$book["isbn"].".PNG")){
			$a="<a href=\"book.php?id=".$book["id"]."\"><img class=\"hov\" src=\"../pictures/".$book["isbn"].".PNG\"> </img></a>"; //adding a picture of the book
			$b=" class=\"picBook\" ";
			$c="";
		} else {
*/
		$a = "<a href=\"book.php?id=".$book["id"]."\">"; //creates a link for each
		$b=""; 
		//$c="</a>"; /*if you don't have a picture for the book a link is still made to the sales page*/
	//}
		//$add=" style=\""; //add style
		//$add.="\""; //end style

		$output .= "<b_name class=\"header\">".$book["name"]."</b_name>"; //information for each book, each book occupies a square
		$output .= "<br />Publisher: ";
		$output .= $book["publisher"];
		$output .= "<br />ISBN: ";
		$output .= $book["isbn"];
		$output .= "<br />SBT Price: ";
		$output .= $book["sbt_price"];
		$output .= " <br /></a>";	
/*
...
function isbn_search() //searches for books with the ISBN #
{
	...
	if (isset($_POST['search'])) //the search button is called 'search,' after it is hit, run the statements below
	{
		...
		return "Your search matched no results. Please try again.";
	}
}
...
function create_message_row($text, $buyer_id, $seller_id, $sell_id)
{
	...
	$query .= "message1, message2, message3, message4, message5, speaker1, speaker2, speaker3, speaker4, speaker5, message_read) ";
	...
	$query .= "VALUES ({$buyer_id}, {$seller_id}, {$book_id}, {$sell_id}, '{$text}', '', '', '', '', 'b', '', '', '', '', 0)";
	/* values put inside this database:
	 * buyer_id: the user id of the buyer
	 * seller_id: the user id of the seller
	 ...
	 * message_read: determines whether the conversation partner has read the new message or not
	 */
...
function unread_message($user_id)
{
	...
	$query .= "FROM messages ";
	$query .= "WHERE buyer_id = {$user_id}";
	$messages_from_buy = mysqli_query($connection, $query);
	
	while ($message = mysqli_fetch_assoc($messages_from_buy))
	{
		for ($i = 5; $i > 0; $i--)
		{
			$speaker_var = "speaker" . $i;
			if ($message[$speaker_var] != NULL)
			{
				if ($message["message_read"] == 0 && $message[$speaker_var] == 's')
					return 1;
				$i = 0;
			}
		}
	...
	$messages_from_sell = mysqli_query($connection, $query);
	
	while ($message = mysqli_fetch_assoc($messages_from_sell))
	{
		for ($i = 5; $i > 0; $i--)
		{
			$speaker_var = "speaker" . $i;
			if ($message[$speaker_var] != NULL)
			{
				if ($message["message_read"] == 0 && $message[$speaker_var] == 'b')
					return 1;
				$i = 0;
			}
		}
...
function update_message_read($user_id, $messages_id)
{
	global $connection;
	...
	while ($message = mysqli_fetch_assoc($messages_from_buy))
	{
		for ($i = 5; $i > 0; $i--)
		{
			...
			if ($message[$speaker_var] != NULL)
			{
				...
				{
					$query = "UPDATE messages ";
					$query .= "SET message_read = 1 ";
					$query .= "WHERE id = {$messages_id}";
					$update = mysqli_query($connection, $query);
				}
				$i = 0;
			}
